...
export function CharacterSelection({ selected, onSelect, onStart, allowedIds, intro, help }: {
  selected: CharacterId
  onSelect: (id: CharacterId) => void
  onStart: () => void
  /** Restrict the roster (third-person experiment offers modeled dolls only). */
  allowedIds?: readonly CharacterId[]
  intro?: string
  help?: string
}) {
  const dolls = allowedIds ? CHARACTER_LIST.filter((doll) => allowedIds.includes(doll.id)) : CHARACTER_LIST
  return <section className="selection-screen" aria-labelledby="selection-title">
    ...
    <p className="selection-intro">{intro ?? 'Six prototype fan-game adaptations. Basic weapons only; these are not verified official kits.'}</p>
    <div className="doll-grid" role="group" aria-label="Playable T-Dolls">
      {dolls.map((doll, index) => <button key={doll.id} className="doll-card" aria-label={`Select ${doll.name}`} aria-pressed={selected === doll.id} onClick={() => onSelect(doll.id)}>
    ...
    <p className="selection-help">{help ?? 'WASD to move · Mouse to aim · Automatic fire when an enemy is in range · Escape to pause'}</p>
