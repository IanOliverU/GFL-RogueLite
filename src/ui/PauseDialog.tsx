...
export function PauseDialog({ simulation }: { simulation: Simulation }) {
  ...
  return <div className="pause-backdrop">
    <section className="pause-card" role="dialog" aria-modal="true" aria-labelledby="pause-title">
      ...
      <h2 id="pause-title">Playground paused</h2>
      ...
      <button ref={button} aria-disabled={reason === 'graphics'} onClick={resume} onKeyDown={(event) => { if (event.key === 'Tab') event.preventDefault() }}>Resume playground <span>↗</span></button>
