...
import { deriveWeapon, isEmpowered } from './progression'
...
export function stepWeapon(world: World, dt: number, aimActive: boolean) {
  ...
  const empowered = isEmpowered(world)
  const pellets = definition.pellets + (empowered ? 4 : 0)
  if (state.cooldown > 1e-8 || world.projectiles.length + pellets > MAX_PROJECTILES) return
  if (state.burstRemaining === 0) state.burstRemaining = definition.burstSize
  for (let pellet = 0; pellet < pellets; pellet++) {
    const angle = pellets === 1 ? 0 : (pellet / (pellets - 1) - 0.5) * definition.spreadDegrees * Math.PI / 180
    ...
    world.projectiles.push({ id: world.nextId++, ...world.player, direction, speed: definition.projectileSpeed, remaining: definition.range, damage: definition.damage * stats.damageMult * (empowered ? 2 : 1), maxHits: definition.maxHits, knockback: definition.knockback * (empowered ? 2 : 1), hitIds: [] })
  }
  world.shots++
  world.volleys++
  state.ammo--
  ...
  state.cooldown = Math.max(-dt, state.cooldown) + (state.burstRemaining > 0 ? definition.burstInterval : definition.interval * stats.intervalMult)
  if (state.ammo === 0) {
